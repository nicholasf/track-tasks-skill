...
def test_build_preflight_shows_file_tokens():
    text = build_preflight_section(500, {'schema.sql': 1200}, 12000, 65536, 215.0)
    assert 'schema.sql' in text
    assert '1,200' in text


def test_build_preflight_shows_complexity_level():
    text = build_preflight_section(1000, {}, 12000, 65536, 215.0)
    assert 'L1' in text


def test_build_preflight_l3_for_large_task():
    text = build_preflight_section(30000, {}, 12000, 65536, 215.0)
    assert 'L3' in text
...
def test_build_preflight_flags_overflow():
    ...
    text = build_preflight_section(55000, {}, 12000, 65536, 215.0)
...
def test_build_preflight_no_files_shows_none_listed():
    text = build_preflight_section(500, {}, 12000, None, None)
...
def test_append_preflight_adds_section(tmp_path):
    task = tmp_path / 'task.md'
    task.write_text(TASK_TEXT)
    append_preflight(str(task), '## Pre-flight\n\n- Spec: 100 tokens\n')
    content = task.read_text()
    assert '## Pre-flight' in content
    assert '100 tokens' in content


def test_append_preflight_replaces_existing_section(tmp_path):
    task = tmp_path / 'task.md'
...
def test_append_preflight_preserves_rest_of_task(tmp_path):
    task = tmp_path / 'task.md'
    task.write_text(TASK_TEXT)
    append_preflight(str(task), '## Pre-flight\n\n- Spec: 100 tokens\n')
    content = task.read_text()
